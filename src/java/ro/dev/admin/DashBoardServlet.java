...
import ro.dev.util.Constants;
import ro.dev.util.GetDateTime;
...
public class DashBoardServlet extends BaseAdminServlet {
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response)
        ...
        List<Order> orderPendingList = orderDao.findAll("pending");
    ...
    private double getTotal() {
        double total = 0;
//        OrderDao orderDao = DatabaseDao.getInstance().getOrderDao();
//        List<Order> orderList = orderDao.findAll("finished");
//        for (Order order : orderList) {
//            List<OrderDetail> orderDetaiList = Database.getInstance().getOrderDetailDao().findAll(order.id);
...
//        }
    ...
    private double sum(List<OrderDetail> orderDetaiList) {
        ...
        for (OrderDetail orderDetail : orderDetaiList) {
            s += orderDetail.quantity * orderDetail.price;
        }
